@section('title', (string) __('user::auth.login.title'))
            ...
            <form wire:submit.prevent="authenticate" class="space-y-6">
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center">
                        ...
                        <label for="remember" class="ml-2 block text-sm text-gray-900">
                            {{ (string) __('pub_theme::auth.login.remember') }}
                        </label>
                    </div>
                    ...
                    @if (Route::has('password.request'))
                        <div class="text-sm">
                            <a href="{{ route('password.request') }}" class="font-medium text-blue-600 hover:text-blue-500">
                                {{ (string) __('pub_theme::auth.login.forgot_password') }}
                            </a>
                        </div>
                    @endif
                </div>
                ...
                <div>
                    <button type="submit" 
                        ...
                        {{ (string) __('pub_theme::auth.login.submit') }}
                    </button>
                </div>
            </form>
                    ...
                    <div class="relative">
                        ...
                        <div class="relative flex justify-center text-sm">
                            <span class="px-2 bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400">
                                {{ (string) __('user::auth.social.title') }}
                            </span>
                        </div>
                    </div>
                        ...
                        @if (config('services.google.client_id'))
                            <a href="{{ route('social.redirect', 'google') }}" 
                               class="w-full inline-flex justify-center py-2 px-4 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm bg-white dark:bg-gray-700 text-sm font-medium text-gray-500 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600">
                                ...
                                <span class="ml-2">{{ (string) __('user::auth.social.google') }}</span>
                            </a>
                        @endif
                        ...
                        @if (config('services.facebook.client_id'))
                            <a href="{{ route('social.redirect', 'facebook') }}"
                               class="w-full inline-flex justify-center py-2 px-4 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm bg-white dark:bg-gray-700 text-sm font-medium text-gray-500 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600">
                                ...
                                <span class="ml-2">{{ (string) __('user::auth.social.facebook') }}</span>
                            </a>
                        @endif
                        ...
                        @if (config('services.github.client_id'))
                            <a href="{{ route('social.redirect', 'github') }}"
                               class="w-full inline-flex justify-center py-2 px-4 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm bg-white dark:bg-gray-700 text-sm font-medium text-gray-500 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600">
                                ...
                                <span class="ml-2">{{ (string) __('user::auth.social.github') }}</span>
                            </a>
                        @endif
                    </div>
                </div>
