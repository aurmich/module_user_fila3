    ...
    <form wire:submit="login" class="space-y-6">
        ...
        <div>
            <label for="email" class="block text-sm font-medium text-gray-700">
                {{ (string) __('user::auth.login.email') }}
            </label>
            <div class="mt-1">
                <input wire:model="email" id="email" type="email" autocomplete="email" required
                    ...
                    placeholder="{{ (string) __('user::auth.login.email_placeholder') }}">
            </div>
            @error('email')
        ...
        <div>
            <label for="password" class="block text-sm font-medium text-gray-700">
                {{ (string) __('user::auth.login.password') }}
            </label>
            <div class="mt-1">
                <input wire:model="password" id="password" type="password" autocomplete="current-password" required
                    class="block w-full appearance-none rounded-md border border-gray-300 px-3 py-2 placeholder-gray-400 shadow-sm focus:border-primary-500 focus:outline-none focus:ring-primary-500 sm:text-sm"
                    placeholder="{{ (string) __('user::auth.login.password_placeholder') }}">
            </div>
            @error('password')
                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
            @enderror
        ...
        <div>
            <button type="submit"
                ...
                {{ (string) __('user::auth.login.submit') }}
            </button>
        </div>
