    ...
    public function registerMailsNotification(): void
    {
        ...
        ResetPassword::toMailUsing(function ($notifiable, string $token): SpatieEmail {
            ...
            return (new MailMessage)
                ...
                ->subject((string) __('user::reset_password.password_reset_subject'))
                ->line((string) __('user::reset_password.password_cause_of_email'))
                ->action((string) __('user::reset_password.reset_password'), url(route('password.reset', $token, false)))
                ->line((string) __('user::reset_password.password_if_not_requested'))
                ->line((string) __('user::reset_password.thank_you_for_using_app'))
                ->salutation((string) __('user::reset_password.regards'));
            */
        ...
        /*
        $salutation = (string) __('user::verify_email.salutation', ['app_name' => $app_name]);
        VerifyEmail::toMailUsing(function (object $notifiable, string $url) use ($salutation): MailMessage {
            return (new MailMessage)
                ->template('user::notifications.email')
                ->subject((string) __('user::verify_email.subject'))
                ->greeting((string) __('user::verify_email.greeting'))
                ->line((string) __('user::verify_email.line1'))
                ->action((string) __('user::verify_email.action'), $url)
                ->line((string) __('user::verify_email.line2'))
