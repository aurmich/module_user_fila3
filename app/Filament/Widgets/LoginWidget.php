        ...
        try {
            ...
            if (! Auth::attempt($attempt_data, $remember)) {
                throw ValidationException::withMessages([
                    'email' => [(string) __('user::messages.credentials_incorrect')],
                ]);
            }
            ...
            Notification::make()
                ->title((string) __('user::messages.login_success'))
                ->success()
                ->send();
            ...
            Notification::make()
                ->title((string) __('user::messages.validation_error'))
                ->body($e->getMessage())
                ->danger()
                ->send();
            ...
            Notification::make()
                ->title((string) __('user::messages.login_error'))
                ->body((string) __('user::messages.login_error'))
                ->danger()
                ->send();
            ...
            $this->addError('email', (string) __('user::messages.login_error'));
        }
    }
}
