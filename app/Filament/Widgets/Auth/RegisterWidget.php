...
class RegisterWidget extends XotBaseWidget
{
            ...
            'user_info' => Section::make()
                ->schema([
                    'first_name' => TextInput::make('first_name')
                        ->label((string) __('user::auth.fields.first_name'))
                        ->required()
                        ->string()
                        ->minLength(2)
                        ->maxLength(255)
                        ->autocomplete('given-name')
                        ->validationAttribute((string) __('user::auth.fields.first_name')),

                    'last_name' => TextInput::make('last_name')
                        ->label((string) __('user::auth.fields.last_name'))
                        ->required()
                        ->string()
                        ->minLength(2)
                        ->maxLength(255)
                        ->autocomplete('family-name')
                        ->validationAttribute((string) __('user::auth.fields.last_name')),

                    'email' => TextInput::make('email')
                        ->label((string) __('user::auth.fields.email'))
                        ->required()
                        ->email()
                        ->maxLength(255)
                        ->unique(User::class, 'email')
                        ->autocomplete('email')
                        ->validationAttribute((string) __('user::auth.fields.email'))
                        ->helperText((string) __('user::auth.help.email')),

                    'password_grid' => Grid::make(2)
                        ->schema([
                            'password' => TextInput::make('password')
                                ->label((string) __('user::auth.fields.password'))
                                ->password()
                                ->required()
                                ->string()
                                ->minLength(12)
                                ...
                                ->validationMessages([
                                    'password.regex' => (string) __('user::auth.validation.password.complexity'),
                                ])
                                ->autocomplete('new-password')
                                ->validationAttribute((string) __('user::auth.fields.password'))
                                ->helperText((string) __('user::auth.help.password'))
                                ->confirmed(),

                            'password_confirmation' => TextInput::make('password_confirmation')
                                ->label((string) __('user::auth.fields.password_confirmation'))
                                ->password()
                                ->required()
                                ->string()
                                ->minLength(12)
                                ->maxLength(255)
                                ->autocomplete('new-password')
                                ->validationAttribute((string) __('user::auth.fields.password_confirmation'))
                                ->dehydrated(false)
                                ->same('password'),
                        ]),
                ]),
    ...
    protected function handleSuccessfulRegistration(User $user): void
    {
        ...
        Notification::make()
            ->title((string) __('user::auth.registration.success'))
            ->success()
            ->send();
    ...
    protected function handleRegistrationError(\Exception $e): void
    {
        ...
        throw new \RuntimeException((string) __('user::auth.registration.error_occurred'));
    }
}
