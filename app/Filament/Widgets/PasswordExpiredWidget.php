...
class PasswordExpiredWidget extends XotBaseWidget implements HasForms
{
    ...
    public function resetPassword(): ?PasswordResetResponse
    {
        ...
        if (! $user || ! ($user instanceof \Illuminate\Database\Eloquent\Model)) {
            $this->addError('current_password', (string) __('user::auth.user_not_found'));
        ...
        if (empty($currentPassword) || empty($newPassword)) {
            $this->addError('current_password', (string) __('user::auth.password_fields_required'));
        ...
        if (! Hash::check($currentPassword, $userPasswordString)) {
            $this->addError('current_password', (string) __('user::auth.password_current_incorrect'));
    ...
    {
        $validation_messages = (string) __('user::validation');
