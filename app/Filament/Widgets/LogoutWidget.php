...
class LogoutWidget extends XotBaseWidget
{
    ...
    protected function redirectAfterLogout(): void
    {
        $redirect = redirect($this->getLocalizedHomeUrl())
            ->with('success', (string) __('user::auth.logout_success'));
    ...
    protected function handleLogoutError(Throwable $e): void
    {
        ...
        $this->isLoggingOut = false;
        Session::flash('error', (string) __('user::auth.logout_error'));
    }
    ...
    {
        return [
            'title' => (string) __('user::auth.logout_title'),
            'description' => (string) __('user::auth.logout_confirmation'),
        ];
    }
}
